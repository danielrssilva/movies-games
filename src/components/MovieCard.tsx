                    ...
                    <div className="w-full flex flex-col gap-8 mb-12 items-center">
                        <div className="group-hover:flex hidden w-full justify-center items-center gap-4">
                            ...
                            <div className='flex gap-2'>
                                {ratings.map((_, index) => (
                                    <AnimatedStar key={index} index={index} person="danny" rating={movie.ratings?.danny} onClick={handleRatingClick} />
                                ))}
                            </div>
                        </div>
                        <div className="group-hover:flex hidden items-center gap-4">
                            ...
                            <div className='flex gap-2'>
                                {ratings.map((_, index) => (
                                    <AnimatedStar key={index} index={index} person="hakush" rating={movie.ratings?.hakush} onClick={handleRatingClick} />
                                ))}
                            </div>
                        </div>
                        <div className="group-hover:flex hidden items-center gap-4">
                            ...
                            <div className='flex gap-2'>
                                {ratings.map((_, index) => (
                                    <AnimatedStar key={index} index={index} person="thaai" rating={movie.ratings?.thaai} onClick={handleRatingClick} />
                                ))}
                            </div>
                        </div>
                    </div>
...
const AnimatedStar = (props: AnimatedStarProps) => {
    ...
    return (
        <AnimatePresence key={`${person}-rating-${index}`}>
            <motion.button onClick={() => onClick(person, index + 1)} className='group/star' whileInView="visible" transition={{ duration: 0.2, delay: index * 0.1 }} variants={StarAnimation} initial="hidden" exit="hidden" viewport={{ once: false }}>
                <Star className={`transition-all duration-75 group-hover/star:stroke-white ${className} ${rating > index ? 'stroke-white' : 'stroke-dark-grey-icon'}`} />
            </motion.button>
        </AnimatePresence>
    )
}
