...
const MovieCard: React.FC<MovieCardProps> = (props) => {
    ...
    return (
        <div className="flex flex-col gap-2 relative flex items-center font-montserrat w-[264px] h-[400px] bg-light-grey rounded-lg group">
            <button onClick={() => onRemove(movie._id)} className='group/delete z-10 absolute -right-4 -top-4 bg-darkest-grey rounded-full p-2 hover:bg-red-900 transition-all duration-300 opacity-0 group-hover:opacity-100'>
            ...
            <span className="absolute h-full w-full bg-gradient-to-t from-black to-transparent rounded-lg"></span>
            <button onClick={() => onUpdate({ ...movie, watched: !movie.watched })} className='absolute -left-8 bottom-8 bg-darkest-grey rounded-full p-4 hover:bg-light-grey transition-all duration-300 opacity-0 group-hover:opacity-100'>
                {movie.watched ?
