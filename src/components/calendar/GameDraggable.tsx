...
const GameDraggable: React.FC<GameDraggableProps> = ({ game }) => {
    ...
    return (
        <div ref={dragRef} style={{ opacity }} className={`flex flex-col gap-2 relative flex items-center justify-center w-[230px] h-[105px] bg-darkest-grey rounded-lg group overflow-visible border-t-[10px] border-[#282828] ${isDragging ? 'opacity-10' : ''}`}>
            ...
            <h3 className="text-white text-[24px] font-bold w-full text-left line-clamp-2 px-8">{game.gameName.split('').map((letter, index) => (
